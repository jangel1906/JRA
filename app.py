...
def normalize_goal(goal: Dict[str, Any]) -> Dict[str, Any]:
    """Ensure a goal is safe for storage/rendering.

    We aggressively coerce fields to strings and clamp color indexes so that
    invalid remote payloads or hand-edited bubbles never break callbacks.
    """

    safe_goal = {
        "id": str(goal.get("id", uuid.uuid4())),
        "title": str(goal.get("title", "Untitled goal")),
        "category": str(goal.get("category", "General")),
        "horizon": str(goal.get("horizon", "Ongoing")),
        "focus": str(goal.get("focus", "")),
        "status": str(goal.get("status", "")),
    }

    try:
        color_index = int(goal.get("color_index", seeded_color_index(safe_goal["id"])))
    except Exception:  # noqa: BLE001 - defensive for unexpected input
        color_index = 0

    safe_goal["color_index"] = color_index % len(COLOR_PALETTE)

    if goal.get("updated_at"):
        safe_goal["updated_at"] = str(goal["updated_at"])
...
RAW_INITIAL_GOALS: List[Dict[str, Any]] = [
    {
        "id": str(uuid.uuid4()),
...
INITIAL_GOALS = [normalize_goal(goal) for goal in RAW_INITIAL_GOALS]


DEFAULT_GOOGLE_WEBAPP_URL = os.getenv("GOOGLE_WEBAPP_URL", "")
DEFAULT_GOOGLE_SHARED_TOKEN = os.getenv("GOOGLE_WEBAPP_TOKEN", "")
...
<html lang=\"en\">
  <head>
    ...
    <link rel=\"apple-touch-icon\" href=\"/assets/icons/goal-bubbles-180.png\">
    <title>{%title%}</title>
    {%favicon%}
    {%css%}
  </head>
...
def allow_google_sites(response):  # pragma: no cover - header tweak
    """Permit embedding inside Google Sites while keeping basic protections."""
        ...
        html.Section(
            ...
            children=[
                html.Div(
                    ...
                    children=[
                        html.Label("Save goals to Google (Apps Script / Sheets)", className="sync-title"),
                        html.P(
                            "Deploy a simple Google Apps Script web app (backed by a Sheet or script storage) and paste the ",
                            "web app URL here. You can protect it with a shared token.",
                            className="sync-help",
                        ),
                        dcc.Input(
                            id="google-webapp-url",
                            placeholder="https://script.google.com/macros/s/<deploy-id>/exec",
                            type="text",
                            value=DEFAULT_GOOGLE_WEBAPP_URL,
                        ),
                        dcc.Input(
                            id="google-webapp-token",
                            placeholder="Optional shared token (kept client-side)",
                            type="password",
                            value=DEFAULT_GOOGLE_SHARED_TOKEN,
                        ),
                        html.Div(
                            className="sync-buttons",
                            children=[
                                html.Button("Save to Google", id="sync-btn"),
                                html.Button("Load from Google", id="load-btn", className="ghost"),
                            ],
                        ),
...
def add_goal(n_clicks: int, text: str, category: str, horizon: str, goals: List[Dict[str, Any]]):
    ...
    updated = [normalize_goal(g) for g in (goals or [])] + [new_goal]
...
def update_grid(goals: List[Dict[str, Any]], categories: List[str], horizons: List[str], term: str):
    goals = [normalize_goal(g) for g in (goals or [])]
...
@callback(
    ...
    State("google-webapp-url", "value"),
    State("google-webapp-token", "value"),
    State("goal-store", "data"),
    prevent_initial_call=True,
)
def sync_to_google(n_clicks: int, url: str, token: str, goals: List[Dict[str, Any]]):
    if not url:
        return "Add your Google web app URL to sync the bubbles."

    safe_goals = [normalize_goal(goal) for goal in (goals or [])]
    headers = {"Content-Type": "application/json; charset=utf-8"}
    if token:
        headers["X-Apps-Script-Token"] = token

    try:
        response = requests.post(
            url,
            json={"goals": safe_goals},
            timeout=10,
            headers=headers,
        )
        if response.ok:
            return "Synced! Your goals were saved to Google."
        return f"Google web app responded with {response.status_code}: {response.text}"
...
def load_from_google(n_clicks: int, url: str, token: str, goals: List[Dict[str, Any]]):
    goals = goals or []
    if not url:
        return goals, "Add your Google web app URL to sync the bubbles."
    ...
    try:
        ...
        if not response.ok:
            return goals, f"Google web app responded with {response.status_code}: {response.text}"

        payload = response.json()
        if isinstance(payload, dict) and "goals" in payload:
        ...
        if isinstance(payload, list):
            ...
            for goal in payload:
                if isinstance(goal, dict):
                    hydrated.append(normalize_goal(goal))
            if hydrated:
                return hydrated, "Loaded goals from Google."
            return goals, "Fetched data, but no valid goals were found to load."
        return goals, "Google data was not a list. Ensure your script returns a JSON list of goals."
    except Exception as exc:  # noqa: BLE001
